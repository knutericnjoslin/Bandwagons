function [p_go]=QRE20_NoCorrection_SecondStageErrors(p_go_0, tau, B2Y, B1Y, a, b)
...
while (ksteps<maxsteps) && (dist>tol)
    ...
    for i=1:1:6
     for j=1:1:20
       ...
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*( p_go(1,j)*B2Y(i) + (1-p_go(1,j))*( p2(j)*B2Y(i) + (1-p2(j))*B1Y(i,a,b) ) );
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*( (p_go(1,j)*5 + (1-p_go(1,j))*( p2(j)*5 + (1-p2(j))*7 ) ) );
       else                %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*( p_go(1,j)*B2Y(i) + (1 - p_go(1,j) )*(p2(j)*B2Y(i) + (1 - p2(j))*B2X(i) ));
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*( (p_go(1,j)*5 + (1 - p_go(1,j))*(p2(j)*5 + (1 - p2(j))*7 ) ) ) ;
       end;
     end;
    end;
    for i=7:1:20
     for j=1:1:20
       if (j <= 6)        %   Here, if agent of type i chooses "go" the agent of type j will not follow. Similarly, if the type j agent goes then the type i agent will not follow 
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*(p2(j)*B2Y + (1 - p2(j))*B1Y(i,a,b)) ) ; 
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*(p2(j)*5 + (1-p2(j))*7 ) ) );
       else   %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*( p2(j)*B2Y(i) + (1 - p2(j))*B1Y(i) ) );
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*( p2(j)*5 + (1 - p2(j))*7 )));   
       end;
     end;
     end;
