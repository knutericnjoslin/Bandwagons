function [p_go]=QRE_FarrellAndSaloner_FlexibleUtilitySpecification(p_go_0, tau, B2Y, B1Y, a, b)
...
%     piexp_stay=zeros(1,10);
    ...
    for j=1:1:10
       ...
       piexp_go(1,i) = piexp_go(1,i) + p_go(1,i)*(1/10)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B1Y(i,a,b));
       piexp_stay(1,i) = piexp_stay(1,i) + (1 - p_go(1,i))*(1/10)*((p_go(1,j)*5 + (1-p_go(1,j))*7));
       else             %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + p_go(1,i)*(1/10)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i));
       piexp_stay(1,i) = piexp_stay(1,i) + (1 - p_go(1,i))*(1/10)*((p_go(1,j)*5 + (1-p_go(1,j))*7));
       end;
    end;
    end;
    ...
    for j=1:1:10
       if j<=3         %   Here, if agent of type i chooses "go" the agent of type j will not follow. Similarly, if the type j agent goes then the type i agent will not follow 
       piexp_go(1,i) = piexp_go(1,i) + p_go(1,i)*(1/10)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B1Y(i,a,b)) ; 
       piexp_stay(1,i) = piexp_stay(1,i) + (1 - p_go(1,i))*(1/10)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*7));
       else             %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + p_go(1,i)*(1/10)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i));
       piexp_stay(1,i) = piexp_stay(1,i) + (1 - p_go(1,i))*(1/10)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*7));
       end;
