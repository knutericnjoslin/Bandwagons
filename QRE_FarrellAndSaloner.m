function [p_go]=QRE_FarrellAndSaloner(p_go_0, tau)
...
%     piexp_stay=zeros(1,10);
    ...
    for j=1:1:10
       ...
       piexp_go(1,i) = piexp_go(1,i) + p_go(1,i)*(1/10)*(p_go(1,j)*(i+2-0.5) + (1-p_go(1,j))*(i-0.5));
       piexp_stay(1,i) = piexp_stay(1,i) + (1 - p_go(1,i))*(1/10)*((p_go(1,j)*5 + (1-p_go(1,j))*7));
       else             %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + p_go(1,i)*(1/10)*(p_go(1,j)*(i+2-0.5) + (1-p_go(1,j))*(i+2-0.5));
       piexp_stay(1,i) = piexp_stay(1,i) + (1 - p_go(1,i))*(1/10)*((p_go(1,j)*5 + (1-p_go(1,j))*7));
       end;
    end;
    end;
    ...
    for j=1:1:10
       if j<=3         %   Here, if agent of type i chooses "go" the agent of type j will not follow. Similarly, if the type j agent goes then the type i agent will not follow 
       piexp_go(1,i) = piexp_go(1,i) + p_go(1,i)*(1/10)*(p_go(1,j)*(i+2-0.5) + (1-p_go(1,j))*(i-0.5)) ; 
       piexp_stay(1,i) = piexp_stay(1,i) + (1 - p_go(1,i))*(1/10)*((p_go(1,j)*(i+2-0.5) + (1-p_go(1,j))*7));
       else             %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + p_go(1,i)*(1/10)*(p_go(1,j)*(i+2-0.5) + (1-p_go(1,j))*(i+2-0.5));
       piexp_stay(1,i) = piexp_stay(1,i) + (1 - p_go(1,i))*(1/10)*((p_go(1,j)*(i+2-0.5) + (1-p_go(1,j))*7));
       end;
