function [p_go]=QRE_FarrellAndSaloner_SecondStageAdjusted_20(p_go_0, tau, B2Y, B1Y, a, b)
...
while (ksteps<maxsteps) && (dist>tol)
    ...
    for i=1:1:6
     for j=1:1:20
       ...
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B1Y(i,a,b));
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*5 + (1-p_go(1,j))*7));
       elseif (j > 6) &&  (j <= 14) %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i));
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*5 + (1-p_go(1,j))*7));
       else
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i));
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*5 + (1-p_go(1,j))*5));    
       end;
     end;
    end;
    for i=7:1:14
     for j=1:1:20
       if (j <= 6)        %   Here, if agent of type i chooses "go" the agent of type j will not follow. Similarly, if the type j agent goes then the type i agent will not follow 
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B1Y(i,a,b)) ; 
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*7));
       elseif(j > 6) &&  (j <= 14)   %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i));
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*7));
       else
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i));
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*5);    
       end;
     end;
     end;
     for i=15:1:20
      for j=1:1:20
       if (j <= 6)        %   Here, if agent of type i chooses "go" the agent of type j will not follow. Similarly, if the type j agent goes then the type i agent will not follow 
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B1Y(i,a,b)) ; 
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B1Y(i,a,b)));
       elseif(j > 6) &&  (j <= 14)   %   for (j > 3) Here the type j agent will follow
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i));
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B1Y(i,a,b)));
       else
       piexp_go(1,i) = piexp_go(1,i) + (1/20)*(p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i));
       piexp_stay(1,i) = piexp_stay(1,i) + (1/20)*((p_go(1,j)*B2Y(i) + (1-p_go(1,j))*B2Y(i)));    
       end;
      end;
     end;
